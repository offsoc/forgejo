...
func TestSyncPushCommits(t *testing.T) {
	...
	t.Run("All commits", func(t *testing.T) {
		...
		NewNotifier().SyncPushCommits(db.DefaultContext, user, repo, &repository.PushUpdateOptions{RefFullName: git.RefNameFromBranch("master")}, pushNotifications())
	...
	t.Run("Only one commit", func(t *testing.T) {
		...
		maxID := unittest.GetCount(t, &activities_model.Action{})
		NewNotifier().SyncPushCommits(db.DefaultContext, user, repo, &repository.PushUpdateOptions{RefFullName: git.RefNameFromBranch("main")}, pushNotifications())
