...
func prepareOpenWithEditorApps(ctx *context.Context) {
	...
	for _, app := range apps {
		...
		if schema == "vscode" || schema == "vscodium" || schema == "jetbrains" {
			iconHTML = svg.RenderHTML(fmt.Sprintf("gitea-open-with-%s", schema), 16, "tw-me-2")
		} else {
			iconHTML = svg.RenderHTML("gitea-git", 16, "tw-me-2") // TODO: it could support user's customized icon in the future
		}
