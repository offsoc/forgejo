...
func TestPackageDebian(t *testing.T) {
	...
	t.Run("UploadWeb", func(t *testing.T) {
		...
		archive := createArchive(packageName, packageVersion, "all")
		...
		require.NoError(t, writer.WriteField("_csrf", GetCSRF(t, session, fmt.Sprintf("%s/-/packages/upload/debian", user.HomeLink()))))
		require.NoError(t, writer.WriteField("repo", "repo1"))
		require.NoError(t, writer.WriteField("distribution", "forgejo"))
		require.NoError(t, writer.WriteField("component", "main"))

		formFile, err := writer.CreateFormFile("file", "test.deb")
		require.NoError(t, err)
		...
		req := NewRequestWithBody(t, "POST", fmt.Sprintf("%s/-/packages/upload/debian/upload", user.HomeLink()), &body).
			SetHeader("Content-Type", writer.FormDataContentType())
		resp := session.MakeRequest(t, req, http.StatusSeeOther)
		assert.Equal(t, fmt.Sprintf("%s/-/packages/debian/%s", user.HomeLink(), packageName), resp.Header().Get("Location"))
	})
}
